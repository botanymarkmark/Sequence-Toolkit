...
def build(parent: ttk.Frame, app) -> ttk.Frame:
    ...
    def detect_r() -> None:
        ...
        # 控件取值必须在**主线程**取完快照再定义 job：job 跑在后台线程，在那里碰 Tk
        # 变量会抛 RuntimeError: main thread is not in main loop（A8 实测：任务直接
        # 失败，界面上只有一句"检测失败"）。所以 rscript.get() 写在 job 外面。
        rscript_value = rscript.get().strip()

        def job(_ctx):
            return _detect_environment(rscript_value)
